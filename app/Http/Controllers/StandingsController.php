...
class StandingsController extends JSONController
{	
        ...
        public function standings($division,$gender,$full_season="",$code="")
	{
		...
		if ($gender == "m" && $division == "III") {
			$cdII = $this->get_confs($gender,"II");
			$conf['northeast-10'] = $cdII['northeast-10'];
		}
		$data = [];
		$datatable = [];
		...
		foreach ($conf as $key => $rec) {
			if ($code == "" or $rec['code'] == $code) {
				$filename = $this->get_jsonfilename("standings",$rec['code'],$gender,$rec['division'],$season,$rec);
