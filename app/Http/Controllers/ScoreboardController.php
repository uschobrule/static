...
class ScoreboardController extends JSONController
{	
		...
		if (1) {//$livejson != null) {
			...
			foreach ($json->data as $rec) {
				...
				if (property_exists($livejson,$game_id)) {
					...
					$game['ls'] = 0;
					$game['last_play'] = "";
                                        if (property_exists($live,"last_play")) {
                                                $game['last_play'] = $live->last_play;
					}
					$game['pp_msg'] = "";
					if (property_exists($live,"pp")) {
                                                if (property_exists($live->pp,"pp_msg")) {
                                                	$game['pp_msg'] = $live->pp->pp_msg." ".$live->pp->pp_sub_msg;
                                        	}
                                        }
					$game['location'] = "";
					$game['pd_suffix'] = "";
	...
	public function gameday_template($template,$json,$code,$page_title,$gender,$division,$full_season,$gamedate,$refreshTime){
		...
		foreach ($json->data as $rec) {
			...
			if ($livejson) {
				...
				if (property_exists($live,"complete")) {
                                        $scroll_status = "live";
					if (property_exists($live,'hscore')) { $game['lhscore'] = $live->hscore;}
					if (property_exists($live,'vscore')) { $game['lvscore'] = $live->vscore;}
					if ($live->complete == "N" && $game_complete == "N") {
                                        	$game['pd'] = $live->period;
						$game['pd_suffix'] = $pdmap[$game['pd']];
						$game['pp_msg'] = "";
						if (property_exists($live,"clock")) {
							$game['pd_time'] = $this->pd_countdown_time($game['pd'],$live->clock,$minutesot,$countdown);
						} else {
							$game['pd_time'] = "";
						}
		...
		if (count($data['data'])) {
                        usort($data['data'], function ($item1, $item2) {
                                return strcmp($item1['gconf_name'].$item1['starttime'],$item2['gconf_name'].$item2['starttime']);
                        });
		}
