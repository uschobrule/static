<div id="boxhead">
@if ($json->header->pp_msg)
<h3>{{$json->header->game_result}} <font color="red">{{$json->header->pp_msg}}</font></h3>
@else
...
@if ($json->game_info->complete == "Y")
<div style="color:red;font-weight:bold;">Live Game Complete</div><br>
@else 
<div style="color:red;font-weight:bold;">Live Game Update P{{$json->game_info->period}} {{$json->game_info->pd_time}}</div>
@if (property_exists($json->game_info,"last_play"))
<b>Last Play:</b> {{$json-> game_info->last_play}}<br>
@endif
<br>
@endif
